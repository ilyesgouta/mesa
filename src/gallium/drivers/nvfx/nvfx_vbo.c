...
static unsigned nvfx_decide_upload_mode(struct pipe_context *pipe, const struct pipe_draw_info *info)
{
	...
	if (info->indexed)
	{
		...
		if (nvfx->screen->index_buffer_reloc_flags && (nvfx->idxbuf.index_size == 2 || nvfx->idxbuf.index_size == 4))
		{
        ...
        if (unlikely(nvfx->screen->trace_draw))
          {
                  ...
                  else if (nvfx->use_index_buffer || !info->indexed)
                          fprintf(stderr, " -> buffer range");
                  else
                          fprintf(stderr, " -> inline indices");
                  fprintf(stderr, " [ivtx %u hvtx %u iidx %u hidx %u] <", inline_cost, hardware_cost, index_inline_cost, index_hardware_cost);
                  for (unsigned i = 0; i < nvfx->vtxelt->num_per_vertex_buffer_infos; ++i)
                  {
                          struct nvfx_per_vertex_buffer_info* vbi = &nvfx->vtxelt->per_vertex_buffer_info[i];
                          struct pipe_vertex_buffer *vb = &nvfx->vtxbuf[vbi->vertex_buffer_index];
...
void nvfx_draw_vbo(struct pipe_context *pipe, const struct pipe_draw_info *info)
{
	...
	//if (nvfx->vtxelt->needs_swtnl || !nvfx_state_validate(nvfx))
...
nvfx_vbo_validate(struct nvfx_context *nvfx)
{
	...
	if(nvfx->use_vertex_buffers)
	{
		...
		if(idx != nvfx->vtxelt->num_elements)
			OUT_RINGp(chan, &nvfx->vtxelt->vtxfmt[idx], elements - idx);
	}
	else
...
nvfx_idxbuf_emit(struct nvfx_context* nvfx, unsigned ib_flags)
{
	...
	OUT_RELOC(chan, bo, nvfx->idxbuf.offset, ib_flags | NOUVEAU_BO_LOW, 0, 0);
			  ...
			  const struct pipe_vertex_element *elements)
{
	...
	//cso->needs_translate = nvfx->screen->force_push;
...
#if 0
	transkey.element[0].input_buffer = 0;
	transkey.element[0].input_format =
			transkey.element[0].output_format = PIPE_FORMAT_R32_FLOAT;
	transkey.element[0].input_offset = 0;
	transkey.element[0].instance_divisor = 0;
	transkey.element[0].output_offset = 0;
	transkey.element[0].type = TRANSLATE_ELEMENT_NORMAL;
        transkey.nr_elements = 1;
        transkey.output_stride = 4;
#endif
