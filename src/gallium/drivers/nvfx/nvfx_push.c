...
#if 1
#define OUT_RING OUT_RING_CHECKED
static __inline__ void OUT_RING_CHECKED(struct nouveau_channel *chan, unsigned data)
{
	assert(AVAIL_RING(chan));
	*chan->cur++ = data;
}
#endif
 ...
 *      if (ctx->edgeflag_attr < 16) {
                float *edgeflag = (float*)((uint8_t *)ctx->attr[ctx->edgeflag_attr].map +
...
//      OUT_RING(chan, RING_3D_NI(NV34TCL_VERTEX_DATA, ctx->vtx_size));
